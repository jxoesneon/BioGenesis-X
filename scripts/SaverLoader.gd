...
# *****************************************************************************
#
# Ported to Godot 4.x for BioGenesis-X.
...
# Lists of properties to persist must be named in constant arrays:
...
signal finished() # await this after calling save_game() or load_game()
...
static func free_procedural_nodes(node: Node, is_root := true) -> void:
	# call with node = root
	if !is_root:
		if "PERSIST_AS_PROCEDURAL_OBJECT" in node and node.PERSIST_AS_PROCEDURAL_OBJECT:
			node.queue_free() # children will also be freed!
			return
	else:
		assert(node is Window)
...
func save_game(save_file: FileAccess, tree: SceneTree) -> void:
	_tree = tree
	...
	if use_thread:
		_thread = Thread.new()
		_thread.start(_threaded_save.bind(save_file))
	else:
		_threaded_save(save_file)

func load_game(save_file: FileAccess, tree: SceneTree) -> void:
	_tree = tree
	_root = _tree.get_root()
	...
	# start generating signals in the loaded game.
	await _tree.process_frame
	await _tree.process_frame
	await _tree.process_frame
	await _tree.process_frame
	...
	if use_thread:
		_thread = Thread.new()
		_thread.start(_threaded_load.bind(save_file))
	else:
...
func debug_log(tree: SceneTree) -> String:
	...
	if debug_print_stray_nodes:
		# Godot 4: orphan/stray node listing is a global helper
		print("Stray Nodes:")
		if OS.has_method("print_orphan_nodes"):
			OS.call("print_orphan_nodes")
		print("***********************")
	if debug_print_tree:
		print("Tree:")
...
func _threaded_save(save_file: FileAccess) -> void:
	_register_tree_for_save(_root)
	if DPRINT:
		print("* Serializing Tree for Save *")
...
func _threaded_load(save_file: FileAccess) -> void:
...
func _register_tree_for_save(node: Node) -> void:
	...
	if node == _current_scene and "PERSIST_AS_PROCEDURAL_OBJECT" in node and node.PERSIST_AS_PROCEDURAL_OBJECT:
...
func _register_and_instance_load_objects() -> void:
	# Instances procedural objects (nodes & references) without data.
	# Indexes all persist objects (procedural and non-procedural) in _objects.
	if DPRINT:
		print("* Registering(/Instancing) Objects for Load *")
	_objects[0] = _root
	var scripts := []
...
func _deserialize_load_objects() -> void:
	if DPRINT:
		print("* Deserializing Objects for Load *")
	for serialized_node in _sfile_serialized_nodes:
